...
export const ValuesColumn = ({ action, allActions, actionIndex, onChange, option }) => {
  ...
  const { notesEsc: holdingsNotesEsc, isFetching: isHoldingsNotesEscLoading } = useHoldingsNotesEsc(tenants,  'action', actionIndex, { enabled: isHoldingsCapability && Boolean(tenants?.length) });
